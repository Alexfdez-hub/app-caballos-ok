...
const files = {
  setup: path.join(testsDir, '007_guardians_concurrency_setup.sql'),
  sessionA: path.join(testsDir, '007_guardians_concurrency_session_a.sql'),
  sessionB: path.join(testsDir, '007_guardians_concurrency_session_b.sql'),
  assert: path.join(testsDir, '007_guardians_concurrency_assert.sql'),
  cleanup: path.join(testsDir, '007_guardians_concurrency_cleanup.sql'),
};
...
async function main() {
  ...
  console.log(
    'Arrangement: session A grants and holds its transaction for 2s; session B starts after A has the row lock and must wait.',
  );
  ...
  if (failures.length > 0) {
    console.error('\n007 guardian concurrency tests FAILED:');
  ...
  console.log('\n007 guardian concurrency tests passed.');
  console.log(
