...
if (/^\\[a-z]/im.test(sql)) {
  console.error('018_center_services_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Center services SQL tests failed via docker exec.', dockerResult);
    }

    console.log('018 center services SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Center services SQL tests failed via psql.', psqlResult);
}
