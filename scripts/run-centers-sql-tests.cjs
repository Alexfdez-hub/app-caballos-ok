...
const relativeTestFile = path.join('supabase', 'tests', '009_centers_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('009_centers_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Centers SQL tests failed via docker exec.', dockerResult);
    }

    console.log('009 centers SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Centers SQL tests failed via psql.', psqlResult);
}
