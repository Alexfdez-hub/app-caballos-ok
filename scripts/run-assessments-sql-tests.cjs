...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '016_rider_assessments_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('016_rider_assessments_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Rider assessments SQL tests failed via docker exec.', dockerResult);
    }

    console.log('016 rider assessments SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Rider assessments SQL tests failed via psql.', psqlResult);
}
