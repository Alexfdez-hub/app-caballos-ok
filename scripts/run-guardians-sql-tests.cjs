...
if (/^\\[a-z]/im.test(sql)) {
  console.error('007_guardians_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Guardian SQL tests failed via docker exec.', dockerResult);
    }

    console.log('007 guardian SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Guardian SQL tests failed via psql.', psqlResult);
}
