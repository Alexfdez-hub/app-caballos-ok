...
const files = {
  setup: path.join(testsDir, '023_sessions_concurrency_setup.sql'),
  sessionA: path.join(testsDir, '023_sessions_concurrency_session_a.sql'),
  sessionB: path.join(testsDir, '023_sessions_concurrency_session_b.sql'),
  assert: path.join(testsDir, '023_sessions_concurrency_assert.sql'),
  cleanup: path.join(testsDir, '023_sessions_concurrency_cleanup.sql'),
};
...
async function main() {
  ...
  console.log(
    'Arrangement: session A starts and holds its transaction for 2s; session B starts after A has the booking lock. Replay is idempotent; exactly one session row must remain.',
  );
  ...
  if (!isStartSuccess(sessionA) || !isStartSuccess(sessionB)) {
    failures.push('Both concurrent start attempts must succeed idempotently.');
  }
  ...
  if (failures.length > 0) {
    console.error('\n023 session concurrency tests FAILED:');
  ...
  console.log('\n023 session concurrency tests passed.');
}
