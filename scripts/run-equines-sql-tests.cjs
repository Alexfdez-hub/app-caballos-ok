...
const relativeTestFile = path.join('supabase', 'tests', '011_equines_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('011_equines_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Equines SQL tests failed via docker exec.', dockerResult);
    }

    console.log('011 equines SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Equines SQL tests failed via psql.', psqlResult);
}
