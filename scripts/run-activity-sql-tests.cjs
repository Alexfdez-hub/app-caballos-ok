...
const relativeTestFile = path.join('supabase', 'tests', '024_equine_activity_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('024_equine_activity_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Equine activity SQL tests failed via docker exec.', dockerResult);
    }

    console.log('024 equine activity SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Equine activity SQL tests failed via psql.', psqlResult);
}
