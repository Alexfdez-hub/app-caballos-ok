...
const relativeTestFile = path.join('supabase', 'tests', '020_calendar_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('020_calendar_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Calendar SQL tests failed via docker exec.', dockerResult);
    }

    console.log('020 calendar SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Calendar SQL tests failed via psql.', psqlResult);
}
