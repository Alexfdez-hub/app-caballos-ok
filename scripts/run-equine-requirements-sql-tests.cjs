...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '017_equine_requirements_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('017_equine_requirements_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Equine requirements SQL tests failed via docker exec.', dockerResult);
    }

    console.log('017 equine requirements SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Equine requirements SQL tests failed via psql.', psqlResult);
}
