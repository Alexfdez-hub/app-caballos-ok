...
const files = {
  setup: path.join(testsDir, '024_equine_activity_concurrency_setup.sql'),
  sessionA: path.join(testsDir, '024_equine_activity_concurrency_session_a.sql'),
  sessionB: path.join(testsDir, '024_equine_activity_concurrency_session_b.sql'),
  assert: path.join(testsDir, '024_equine_activity_concurrency_assert.sql'),
  cleanup: path.join(testsDir, '024_equine_activity_concurrency_cleanup.sql'),
};
...
async function main() {
  ...
  console.log(
    'Arrangement: session A records activity and holds its transaction for 2s; session B records after A has the session lock. Replay is idempotent; exactly one activity row must remain.',
  );
  ...
  try {
    const a = runSession(container, files.sessionA, {
      waitFor: /session_a_after_record/,
      timeoutMs: 20000,
    });
    await a.granted;
    console.log(
  ...
  if (!isStartSuccess(sessionA) || !isStartSuccess(sessionB)) {
    failures.push('Both concurrent record attempts must succeed idempotently.');
  }
  ...
  if (failures.length > 0) {
    console.error('\n024 equine activity concurrency tests FAILED:');
  ...
  console.log('\n024 equine activity concurrency tests passed.');
}
