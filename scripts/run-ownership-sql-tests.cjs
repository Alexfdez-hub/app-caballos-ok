...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '012_equine_ownership_management_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '012_equine_ownership_management_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Ownership SQL tests failed via docker exec.', dockerResult);
    }

    console.log('012 equine ownership SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Ownership SQL tests failed via psql.', psqlResult);
}
