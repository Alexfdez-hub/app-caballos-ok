...
if (/^\\[a-z]/im.test(sql)) {
  console.error('027_storage_policies_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Storage SQL tests failed via docker exec.', dockerResult);
    }

    console.log('027 storage SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Storage SQL tests failed via psql.', psqlResult);
}
