...
const relativeTestFile = path.join('supabase', 'tests', '026_audit_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('026_audit_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Audit SQL tests failed via docker exec.', dockerResult);
    }

    console.log('026 audit SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Audit SQL tests failed via psql.', psqlResult);
}
