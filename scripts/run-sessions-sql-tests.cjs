...
const relativeTestFile = path.join('supabase', 'tests', '023_sessions_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('023_sessions_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Sessions SQL tests failed via docker exec.', dockerResult);
    }

    console.log('023 sessions SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Sessions SQL tests failed via psql.', psqlResult);
}
