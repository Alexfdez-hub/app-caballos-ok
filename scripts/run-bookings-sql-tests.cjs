...
const relativeTestFile = path.join('supabase', 'tests', '021_bookings_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('021_bookings_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Bookings SQL tests failed via docker exec.', dockerResult);
    }

    console.log('021 bookings SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Bookings SQL tests failed via psql.', psqlResult);
}
