...
if (/^\\[a-z]/im.test(sql)) {
  console.error('015_qualifications_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Qualifications SQL tests failed via docker exec.', dockerResult);
    }

    console.log('015 qualifications SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Qualifications SQL tests failed via psql.', psqlResult);
}
