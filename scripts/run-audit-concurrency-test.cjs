...
const files = {
  setup: path.join(testsDir, '026_audit_concurrency_setup.sql'),
  sessionA: path.join(testsDir, '026_audit_concurrency_session_a.sql'),
  sessionB: path.join(testsDir, '026_audit_concurrency_session_b.sql'),
  assert: path.join(testsDir, '026_audit_concurrency_assert.sql'),
  cleanup: path.join(testsDir, '026_audit_concurrency_cleanup.sql'),
};
...
async function main() {
  ...
  console.log(
    'Arrangement: session A reports an incident and holds its transaction for 2s; session B reports after A has the session lock. Both inserts succeed; two incidents and two incident_reported audits must remain.',
  );
  ...
  try {
    const a = runSession(container, files.sessionA, {
      waitFor: /session_a_after_incident/,
      timeoutMs: 20000,
    });
    await a.granted;
    console.log(
  ...
  if (!isStartSuccess(sessionA) || !isStartSuccess(sessionB)) {
    failures.push('Both concurrent incident reports must succeed.');
  }
  ...
  if (failures.length > 0) {
    console.error('\n026 audit concurrency tests FAILED:');
  ...
  console.log('\n026 audit concurrency tests passed.');
}
