...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '019_zero_sessions_authorizations_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '019_zero_sessions_authorizations_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Zero sessions SQL tests failed via docker exec.', dockerResult);
    }

    console.log('019 zero sessions SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Zero sessions SQL tests failed via psql.', psqlResult);
}
