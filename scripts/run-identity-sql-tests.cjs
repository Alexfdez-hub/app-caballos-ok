...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '006_identity_integration_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '006_identity_integration_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Identity SQL tests failed via docker exec.', dockerResult);
    }

    console.log('006 identity SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Identity SQL tests failed via psql.', psqlResult);
}
