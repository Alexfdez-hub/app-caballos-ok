...
const relativeTestFile = path.join('supabase', 'tests', '025_reviews_incidents_test.sql');
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('025_reviews_incidents_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Reviews/incidents SQL tests failed via docker exec.', dockerResult);
    }

    console.log('025 reviews/incidents SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Reviews/incidents SQL tests failed via psql.', psqlResult);
}
