...
if (/^\\[a-z]/im.test(sql)) {
  console.error('029_critical_audit_test.sql contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('029 critical audit SQL tests failed via docker exec.', dockerResult);
    }

    console.log('029 critical audit SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('029 critical audit SQL tests failed via psql.', psqlResult);
}
