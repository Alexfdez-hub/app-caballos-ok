...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '008_rider_profiles_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Rider SQL tests failed via docker exec.', dockerResult);
    }

    console.log('008 rider profile SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Rider SQL tests failed via psql.', psqlResult);
}
