...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '010_center_memberships_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '010_center_memberships_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Membership SQL tests failed via docker exec.', dockerResult);
    }

    console.log('010 center membership SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Membership SQL tests failed via psql.', psqlResult);
}
