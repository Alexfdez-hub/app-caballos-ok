...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '022_booking_functions_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error('022_booking_functions_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Booking-functions SQL tests failed via docker exec.', dockerResult);
    }

    console.log('022 booking-functions SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Booking-functions SQL tests failed via psql.', psqlResult);
}
