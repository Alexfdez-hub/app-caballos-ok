...
const relativeTestFile = path.join(
  'supabase',
  'tests',
  '030_security_regression_test.sql',
);
...
if (/^\\[a-z]/im.test(sql)) {
  console.error(
    '030_security_regression_test.sql still contains psql meta-commands.',
  );
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Security regression SQL tests failed via docker exec.', dockerResult);
    }

    console.log('030 security regression SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Security regression SQL tests failed via psql.', psqlResult);
}
