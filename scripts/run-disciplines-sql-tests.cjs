...
if (/^\\[a-z]/im.test(sql)) {
  console.error('014_disciplines_test.sql still contains psql meta-commands.');
  process.exit(1);
}
...
if (succeeded(dockerNames)) {
  ...
  if (container) {
    ...
    if (!succeeded(dockerResult)) {
      fail('Disciplines SQL tests failed via docker exec.', dockerResult);
    }

    console.log('014 disciplines SQL tests passed.');
    process.exit(0);
  }
}
...
if (!succeeded(psqlResult)) {
  fail('Disciplines SQL tests failed via psql.', psqlResult);
}
