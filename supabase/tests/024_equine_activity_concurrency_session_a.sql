-- Session A: record, hold the session row lock for 2s, commit.
...
select clock_timestamp() as session_a_before_record;
select public.record_equine_activity(current_setting('app.conc_session_id')::uuid) as session_a_activity_id;
select clock_timestamp() as session_a_after_record;
select pg_sleep(2);
