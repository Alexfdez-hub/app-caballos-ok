...
import { listMyEquineManagementAssignments } from './ownershipService';
import type { EquineManagementAssignment } from './types';

export function useMyEquineManagement() {
  const [rows, setRows] = useState<EquineManagementAssignment[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const nextRows = await listMyEquineManagementAssignments();
      if (
